...
if __name__ == "__main__":
    ...
    xgb_result_set1.to_csv('../stacking/level2_train_20161025_feature_set7.csv',index = False)
    rf_result_set1.to_csv('../stacking/level2_train_20161025_feature_set8.csv',index = False)

    #the xgb: 0.454820517003
    #the rf: 0.418200785656
    #the xgb, rf: 0.455850191208
    xgb_result_set1_test.columns = ['pred']
    xgb_result_set1_test.to_csv('../stacking/level2_test_20161025_feature_set7.csv',index = False)
    rf_result_set1_test.columns = ['pred']
    rf_result_set1_test.to_csv('../stacking/level2_test_20161025_feature_set8.csv',index = False)
    ...
    y_pred_final = (((test_rf + test_xgb)/2 > threshold3*0.95).astype(int))
    submission = pd.read_csv('../input/sample_submission.csv')
    submission['Response'] = y_pred_final
    print(sum(y_pred_final))
    submission[['Id', 'Response']].to_csv('../submission/xgboost_20161026_rf_xgb_whole_data_submit2.csv', index=False)
