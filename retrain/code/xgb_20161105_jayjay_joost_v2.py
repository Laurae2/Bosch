...
if __name__ == "__main__":
    ...
    train = feather.read_dataframe('../input/joost_jay_v2/train.feather')
    test = feather.read_dataframe('../input/joost_jay_v2/test.feather')
    ...
    for i in range(1, 6):
        ...
        print("the auc is", (auc(y_valid, xgb_pred)))
    ...
    a = a.reset_index()
    a.to_csv('../log/joost_jay_v2_xgb_20161105_importance.csv', index=False)
    ...
    print("the xgb:",best_mcc,sum(y_pred),roc_auc_score(train.Y.values,xgb_result_set1['pred']))
    best_proba, best_mcc, y_pred = eval_mcc(train.Y.values, rf_result_set1['pred'], True)
    print("the rf:", best_mcc, sum(y_pred), roc_auc_score(train.Y.values, rf_result_set1['pred']))
    best_proba, best_mcc, y_pred = eval_mcc(train.Y.values, final_result_set1['pred'], True)
    print("the xgb,rf:",best_mcc,sum(y_pred),roc_auc_score(train.Y.values,final_result_set1['pred'].values))


    xgb_result_set1.to_csv('../pushing/level1_train_20161105_feature_set9.csv',index = False)
    xgb_result_set1_test.columns = ['pred']
    xgb_result_set1_test.to_csv('../pushing/level1_test_20161105_feature_set9.csv',index = False)
    rf_result_set1_test.columns = ['pred']
    rf_result_set1.to_csv('../pushing/level1_train_20161105_feature_set10.csv', index=False)
    rf_result_set1_test.to_csv('../pushing/level1_test_20161105_feature_set10.csv',index = False)

    #the xgb: 0.454820517003
    #the rf: 0.418200785656
    #the xgb, rf: 0.455850191208
    xgb_result_set1 = pd.read_csv('../pushing/level1_train_20161105_feature_set9.csv')


    clf_xgb = xgb.train(params, dtrain, num_boost_round=280)
    test_xgb = clf_xgb.predict(dtest)


    y_pred_final = (((test_xgb) > (0.347129)*0.94).astype(int))
    submission = pd.read_csv('../input/sample_submission.csv')
    submission['Response'] = y_pred_final
    print(sum(y_pred_final))
    submission[['Id', 'Response']].to_csv('../submission/xgboost_20161105_v1_jayjay_joost_v2.csv', index=False)
    #LB:0.48721
