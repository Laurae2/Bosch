...
if __name__ == "__main__":
    ...
    train = feather.read_dataframe('../input/jay/train.feather')
    test = feather.read_dataframe('../input/jay/test.feather')
    ...
    df = pd.DataFrame()
    df['pred'] = rf_result_set1['pred'] -rf_result_set1['pred'].mean()
    df['rf'] = rf_result_set1['pred']
    df['xgb'] = xgb_result_set1['pred']
    df['xgb_rf'] = xgb_result_set1['pred'] -xgb_result_set1['pred'].mean() + rf_result_set1['pred'] - rf_result_set1['pred'].mean()
    df['f'] = final_result_set1['pred']
    df['x'] = xgb_result_set1['pred'] * 0.3 + 0.7*rf_result_set1['pred']

    print("the overall xgb auc is {}".format(roc_auc_score(train.Y.values, df['xgb_rf'])))
    print("the overall xgb auc is {}".format(roc_auc_score(train.Y.values,xgb_result_set1['pred'])))

    best_proba, best_mcc, y_pred = eval_mcc(train.Y.values, final_result_set1['pred'], True)
    print("the xgb,rf:",best_mcc)
    best_proba, best_mcc, y_pred = eval_mcc(train.Y.values, df['xgb_rf'], True)
    print("the xgb,rf:",best_mcc)
    best_proba, best_mcc, y_pred = eval_mcc(train.Y.values, df['xgb'], True)
    print("the xgb,rf:",best_mcc)

    xgb_result_set1.to_csv('../pushing/level1_train_20161103_feature_set1.csv',index = False)
    xgb_result_set1_test.to_csv('../pushing/level1_test_20161103_feature_set1.csv', index=False)
    rf_result_set1.to_csv('../pushing/level1_train_20161103_feature_set3.csv',index = False)
    rf_result_set1_test.to_csv('../pushing/level1_test_20161103_feature_set3.csv', index=False)
    ...
    gbm = pd.read_csv('../pushing/20161103_lightGBM_train_feature_set2.csv')
    df = pd.DataFrame()
    df['gbm'] = gbm
    df['xgb'] = xgb_result_set1['pred'].values
    df['y'] = train.Y
    df.sort('gbm',ascending = False,inplace = True)
    ...
    xgb_result_set1_test.to_csv('../stacking_new/level1_test_20161029_feature_set9.csv',index = False)
