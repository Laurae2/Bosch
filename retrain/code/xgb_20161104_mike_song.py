...
if __name__ == "__main__":
    ...
    train = feather.read_dataframe('../input/song/tr_song.feather')
    test = feather.read_dataframe('../input/song/te_song.feather')
    ...
    for i in range(1, 6):
        ...
        clf_xgb = xgb.train(params, dtr, num_boost_round=1000, evals=watchlist, verbose_eval=10,
    ...
    xgb_result_set1.to_csv('../pushing/level1_train_20161104_feature_set6.csv',index = False)
    xgb_result_set1_test.columns = ['pred']
    xgb_result_set1_test.to_csv('../pushing/level1_test_20161104_feature_set6.csv', index=False)
    feature_importance.columns = ['feature','score']
    a = feature_importance.groupby('feature')['score'].sum()
    a.sort(ascending = False)
    a= a.reset_index()
    a.to_csv('../log/mike_song_xgb_20161104_importance.csv',index =False)

    #rf_result_set1_test.columns = ['pred']
    #rf_result_set1_test['pred'] = rf_result_set1_test
    #rf_result_set1.to_csv('../pushing/level1_train_20161104_feature_set7.csv',index = False)
    #rf_result_set1_test.to_csv('../pushing/level1_test_20161104_feature_set7.csv',index = False)
