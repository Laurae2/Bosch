...
if __name__ == "__main__":
    ...
    extra_v1 = pd.read_csv('../input/extra_var_v1.csv')
    extra_v2 = pd.read_csv('../input/extra_var_v2.csv')
    extra_v3 = pd.read_csv('../input/extra_var_v1_S34.csv')
    extra_v4 = pd.read_csv('../input/extra_var_v4.csv')
    extra_v5 = pd.read_csv('../input/extra_var_v4bis.csv')
    ...
    #train = pd.concat([train_stack,train],axis = 1)
    #test = pd.concat([test_stack, test], axis=1)

    train = train.merge(extra_v1,on = 'Id')
    train = train.merge(extra_v2, on='Id')
    train = train.merge(extra_v3, on='Id')
    train = train.merge(extra_v4, on='Id',how = 'left')
    train = train.merge(extra_v5, on='Id')
    train = train.merge(tr_feather_set1[['magic3','Id','magic4','magic5','magic6']],on = 'Id')

    test = test.merge(extra_v1,on = 'Id')
    test = test.merge(extra_v2, on='Id')
    test = test.merge(extra_v3, on='Id')
    test = test.merge(extra_v4, on='Id',how = 'left')
    test = test.merge(extra_v5, on='Id')
    test = test.merge(te_feather_set1[['magic3', 'Id', 'magic4','magic5','magic6']], on='Id')
    del extra_v1,extra_v2,extra_v4,extra_v3,extra_v5
    import gc
    gc.collect()
    ...
    xgb_result_set1.to_csv('../stacking_new/level1_train_20161029_feature_set1.csv',index = False)
    rf_result_set1.to_csv('../stacking_new/level1_train_20161029_feature_set2.csv',index = False)
    ...
    xgb_result_set1_test.to_csv('../stacking_new/level1_test_20161029_feature_set1.csv',index = False)
    rf_result_set1_test.columns = ['pred']
    rf_result_set1_test['pred'] = test_rf
    rf_result_set1_test.to_csv('../stacking_new/level1_test_20161029_feature_set2.csv',index = False)
