...
my_script_is_using <- "E:/Laurae/20161109_xgb_laurae"
...
train <- read_feather("Shubin/retrain_material/tr_stack1.feather")
test <- read_feather("Shubin/retrain_material/te_stack1.feather")
train$Y <- NULL

train_mini <- fread("Mike/0 - Data Files/train_eng.csv")
test_mini <- fread("Mike/0 - Data Files/test_eng.csv")
train_mini$Id <- NULL
train_mini$Response <- NULL
test_mini$Id <- NULL
setDT(train)
setDT(test)

train <- DTcbind(train, train_mini)
test <- DTcbind(test, test_mini)
rm(train_mini, test_mini)
gc()
...
for (i in 1:length(folds)) {
  ...
  temp_model <- xgb.train(data = dval1,
                          ...
                          #gamma = 20,
                          subsample = 0.9375316,
                          colsample_bytree = 0.8332737,
                          min_child_weight = 42.51029,
                          booster = "gbtree",
