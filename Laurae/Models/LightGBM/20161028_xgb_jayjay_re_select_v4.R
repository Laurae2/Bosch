...
my_script_is_using <- "E:/Laurae/20161028_xgb_jayjay_re_select_v4"
...
data <- data.frame(matrix(nrow = 2367495, ncol = 0))
data$xgb <- c(fread("Laurae/20161028_lgbm_jayjay_re_select_v4/aaa_stacker_preds_train_headerY.csv")$x, fread("Laurae/20161028_lgbm_jayjay_re_select_v4/aaa_stacker_preds_test_headerY.csv")$x)
data$rf <- c(fread("Laurae/20161028_rf_jayjay_re_select_v4/aaa_stacker_preds_train_headerY.csv")$x, fread("Laurae/20161028_rf_jayjay_re_select_v4/aaa_stacker_preds_test_headerY.csv")$x)

data$xgb_2 <- data$xgb * data$xgb
data$rf_2 <- data$rf * data$rf

data_temp <- data
for (i in 1:(ncol(data) - 1)) {
  for (j in i:ncol(data)) {
    data_temp[, paste(colnames(data)[i], colnames(data)[j], sep = "__")] <- data[, i] * data[, j]
  }
}

spooky <- numeric(ncol(data_temp))
for (i in 1:ncol(data_temp)) {
  spooky[i] <- fastdigest(data_temp[, i])
  cat("Iteration ", i, " --- Uniques: ", length(unique(spooky[1:i])), " ---\n", sep = "")
  if (i %% 20) {gc(verbose = FALSE)}
}
data_temp <- data_temp[, which(!duplicated(spooky))]
gc()
...
dtrain <- xgb.DMatrix(data = data_temp[1:1183747, ], label = label)
gc()
dtest <- xgb.DMatrix(data = data_temp[1183748:2367495, ])
gc()
...
for (i in 1:length(folds)) {
  ...
  gc(verbose = FALSE)
  predictions1[folds[[i]]] <- predict(temp_model, dval2, ntreelimit = 0)
  gc(verbose = FALSE)
  predictions2 <- predictions2 + (0.2 * predict(temp_model, dtest, ntreelimit = 0))
  gc(verbose = FALSE)
...
predictedValues <- predict(temp_model, dtest, ntreelimit = 0)
