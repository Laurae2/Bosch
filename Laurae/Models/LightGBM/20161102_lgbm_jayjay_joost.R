...
my_script_is_using <- "E:/Laurae/20161102_lgbm_jayjay_joost"
...
AnalysisFunc <- function(lgbm, label, folds, validationValues, predictedValuesCV, predictedValues) {
  ...
  cat("Average MCC on all data (5 fold): ", mcc_fixed(y_prob = validationValues, y_true = label, prob = mean(temp_thresh)), ", threshold=", mean(temp_mcc), "\n", sep = "")
