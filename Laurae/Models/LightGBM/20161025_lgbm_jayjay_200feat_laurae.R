...
my_script_is_using <- "E:/Laurae/20161025_lgbm_jayjay_200feat_laurae"
...
numeric_features <- which(colnames(train_numeric) %in% numeric_features)
train_numeric <- DTcolsample(train_numeric, kept = numeric_features, remove = FALSE)

gc()
test_numeric <- readRDS("datasets/test_numeric.rds")
test_numeric <- DTcolsample(test_numeric, kept = numeric_features, remove = FALSE)
...
# 301 features to 253
i <- 0
spooky <- pbapply(Date_features, 2, function(x) {r <- ifelse((min(x, na.rm = TRUE) == max(x, na.rm = TRUE)), TRUE, FALSE); i <<- i + 1; if (!i %% 20) {gc(verbose = FALSE)}; return(r)})
gc()
...
train_date <- Date_features[1:1183747, ]
test_date <- Date_features[1183748:2367495, ]
rm(Date_features)
gc()


# Merge Laurae's numeric+date

train_numeric <- DTcbind(train_numeric, train_date, low_mem = TRUE, collect = 20, silent = FALSE)
test_numeric <- DTcbind(test_numeric, test_date, low_mem = TRUE, collect = 20, silent = FALSE)
rm(train_date, test_date)
gc()

# 478 features as there are duplicates
train <- DTcbind(train, train_numeric, low_mem = TRUE, collect = 20, silent = FALSE)
test <- DTcbind(test, test_numeric, low_mem = TRUE, collect = 20, silent = FALSE)
rm(train_numeric, test_numeric)
gc()

# 478 features to 466 features
