...
my_script_is_using <- "E:/Laurae/20161028_rf_jayjay_re_select_v4"
...
date1 <- readRDS("Laurae/jayjay_features/Laurae_All_30_60_100_165_censored_mean.rds")
date2 <- readRDS("Laurae/jayjay_features/Laurae_All_30_60_100_165_censored_std.rds")
date1 <- DTcbind(date1, date2, low_mem = TRUE, collect = 20, silent = FALSE)
date1[, Id := NULL]
date_no_overfit <- c(grep("100", colnames(date1)), grep("165", colnames(date1)))
date1 <- DTcolsample(date1, date_no_overfit)
train_date <- DTsubsample(date1, 1:1183747)
test_date <- DTsubsample(date1, 1183748:2367495)
rm(date1, date2)
gc()

train <- DTcbind(train, train_date)
test <- DTcbind(test, test_date)
...
fwrite(train, file.path(my_script_is_using, "train.csv"), verbose = TRUE)
fwrite(test, file.path(my_script_is_using, "test.csv"), verbose = TRUE)
rm(train, test)
gc()
train <- h2o.importFile(file.path(my_script_is_using, "train.csv"))
test <- h2o.importFile(file.path(my_script_is_using, "test.csv"))

train$Response <- as.factor(train$Response)


temp_model <- h2o.randomForest(x = 1:896,
                               y = "Response",
...
validationValues <- h2o.cross_validation_holdout_predictions(temp_model)
predictedValues <- h2o.predict(temp_model, test)
validationValues <- as.data.frame(validationValues)$p1
predictedValues <- as.data.frame(predictedValues)$p1
...
AnalysisFunc(lgbm = NA,
             ...
             predictedValuesCV = predictedValues,
             predictedValues = NA)
