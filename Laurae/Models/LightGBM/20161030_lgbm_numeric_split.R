...
my_script_is_using <- "E:/Laurae/20161030_lgbm_numeric_split"
...
test <- readRDS("datasets/test_numeric.rds")
label <- readRDS("datasets/labels.rds")
clusters <- readRDS("Laurae/DatesSpotting/numeric_date_split.rds")

train_ex <- data.table(Id = as.integer(train$Id))
test_ex <- data.table(Id = as.integer(test$Id))
train[, Response := NULL]
train[, Id := NULL]
test[, Id := NULL]
clusters[, Id := NULL]
...
# Just to check the minimal value
# minimum_numeric <- 0
# for (i in colnames(train)) {
#   minimum_numeric <- ifelse(min(train[[i]], na.rm = TRUE) < minimum_numeric, min(train[[i]], na.rm = TRUE), minimum_numeric)
#   gc(verbose = FALSE)
# }

# options(warn = 2) # For debugging purposes only
# options(warn = 1) # For production purposes
# Low memory optimizations
for (i in colnames(train)) {
  train[, (i) := as.integer((2 + train[[i]]) * 1000)]
  cat("Done ", i, ".\n", sep = "")
}
gc()
for (i in colnames(test)) {
  test[, (i) := as.integer((2 + test[[i]]) * 1000)]
  cat("Done ", i, ".\n", sep = "")
}
gc()
...
# Do numeric separation
k <- 0
columns <- copy(colnames(clusters))
for (i in columns) {
  ...
  for (j in unique(clusters[[i]])) {
    
    if (!(j == -1)) {
      
      temp_val <- rep(0, 1183747)
      temp_bool <- (clusters[[i]][1:1183747] == j)
      temp_val[temp_bool] <- train[[i]][temp_bool]
      train_ex[, (paste(i, "_", j, sep = "")) := as.integer(temp_val)]
      temp_val <- rep(0, 1183748)
      temp_bool <- (clusters[[i]][1183748:2367495] == j)
      temp_val[temp_bool] <- test[[i]][temp_bool]
      test_ex[, (paste(i, "_", j, sep = "")) := as.integer(temp_val)]
      
    } else {
      
      temp_val <- rep(1, 1183747)
      temp_bool <- (clusters[[i]][1:1183747] == j)
      temp_val[temp_bool] <- 0
      train_ex[, (paste(i, "_", j, sep = "")) := as.integer(1 - temp_val)]
      temp_val <- rep(1, 1183748)
      temp_bool <- (clusters[[i]][1183748:2367495] == j)
      temp_val[temp_bool] <- 0
      test_ex[, (paste(i, "_", j, sep = "")) := as.integer(1 - temp_val)]
      
    }
    
    l <- l + 1
    cat("\rDoing ", i, "(", j, "). [", l, "]", sep = "")
  ...
  test[[i]] <- NULL
  clusters[[i]] <- NULL
  cat("\rDone ", i, " (clusters: ", l, ").......\n", sep = "")
  gc(verbose = FALSE)
  
}

saveRDS(train_ex, "Laurae/DatesSpotting/train_splitted.rds")
saveRDS(test_ex, "Laurae/DatesSpotting/test_splitted.rds")
saveRDS(train, "Laurae/DatesSpotting/train_isolated.rds")
saveRDS(test, "Laurae/DatesSpotting/test_isolated.rds")
fwrite(train_ex, "Laurae/DatesSpotting/train_splitted.csv", verbose = TRUE)
fwrite(test_ex, "Laurae/DatesSpotting/test_splitted.csv", verbose = TRUE)
fwrite(train, "Laurae/DatesSpotting/train_isolated.csv", verbose = TRUE)
fwrite(test, "Laurae/DatesSpotting/test_isolated.csv", verbose = TRUE)

# train_ex <- readRDS("Laurae/DatesSpotting/train_splitted.rds")
# test_ex <- readRDS("Laurae/DatesSpotting/test_splitted.rds")
# train <- readRDS("Laurae/DatesSpotting/train_isolated.rds")
# test <- readRDS("Laurae/DatesSpotting/test_isolated.rds")

DTfillNA(train, value = 0, low_mem = TRUE, collect = 20, silent = FALSE)
DTfillNA(test, value = 0, low_mem = TRUE, collect = 20, silent = FALSE)

spooky <- numeric(ncol(train_ex))
for (i in 1:ncol(train_ex)) {
  spooky[i] <- fastdigest(train_ex[[i]])
  cat("Iteration ", i, " --- Uniques: ", length(unique(spooky[1:i])), " ---\n", sep = "")
  if (!(i %% 20)) {gc(verbose = FALSE)}
}
train_ex <- DTcolsample(train_ex, kept = which(!duplicated(spooky)), remove = FALSE)
gc()
test_ex <- DTcolsample(test_ex, kept = which(!duplicated(spooky)), remove = FALSE)
gc()
...
train <- DTcolsample(train, kept = which(!duplicated(spooky)), remove = FALSE)
gc()
test <- DTcolsample(test, kept = which(!duplicated(spooky)), remove = FALSE)
gc()

train_ex <- DTcbind(train_ex, train, low_mem = TRUE, collect = 20, silent = FALSE)
test_ex <- DTcbind(test_ex, test, low_mem = TRUE, collect = 20, silent = FALSE)
rm(train, test)
gc()

saveRDS(train_ex, "Laurae/DatesSpotting/train_dedup.rds")
saveRDS(test_ex, "Laurae/DatesSpotting/test_dedup.rds")
fwrite(train_ex, "Laurae/DatesSpotting/train_dedup.csv", verbose = TRUE)
fwrite(test_ex, "Laurae/DatesSpotting/test_dedup.csv", verbose = TRUE)
...
#train <- readRDS("Laurae/DatesSpotting/train_dedup.rds")
#test <- readRDS("Laurae/DatesSpotting/test_dedup.rds")
#train[, Id := NULL]
#test[, Id := NULL]
...
# Pre-export to lower memory usage...

temp_folds <- lgbm.cv.prep(y_train = label,
                           x_train = train,
                           x_test = NA,
