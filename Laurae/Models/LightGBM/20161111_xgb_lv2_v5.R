...
my_script_is_using <- "E:/Laurae/20161111_xgb_lv2_v5"
...
train <- data.table(Empty = rep(0, 1183747))
test <- data.table(Empty = rep(0, 1183748))

train[, "xgb_jay_joost"] <- fread("Laurae/20161110_xgb_jayjoost_fix2/aaa_stacker_preds_train_headerY_scale.csv")$x
test[, "xgb_jay_joost"] <- fread("Laurae/20161110_xgb_jayjoost_fix2/aaa_stacker_preds_test_headerY_scale.csv")$x
train[, "gbm_jay_joost"] <- fread("Laurae/20161111_lgbm_jayjoost/aaa_stacker_preds_train_headerY_scale.csv")$x
test[, "gbm_jay_joost"] <- fread("Laurae/20161111_lgbm_jayjoost/aaa_stacker_preds_test_headerY_scale.csv")$x
train[, "rf_jay_joost"] <- fread("Laurae/20161111_rf_jayjoost/aaa_stacker_preds_train_headerY_scale.csv")$x
test[, "rf_jay_joost"] <- fread("Laurae/20161111_rf_jayjoost/aaa_stacker_preds_test_headerY_scale.csv")$x
...
for (i in 1:(6 - 1)) {
  for (j in i:6) {
    train[[paste0(colnames(train)[i], "_x_", colnames(train)[j])]] <- train[[i]] * train[[j]]
    test[[paste0(colnames(test)[i], "_x_", colnames(test)[j])]] <- test[[i]] * test[[j]]
  }
  cat("Done ", i, "\n")
}

train_data <- readRDS("Laurae/20161031_xgb_stationlearner/xgb_train.rds")
test_data <- readRDS("Laurae/20161031_xgb_stationlearner/xgb_test.rds")

train <- DTcbind(train, train_data, low_mem = TRUE, collect = 32, silent = FALSE)
test <- DTcbind(test, test_data, low_mem = TRUE, collect = 32, silent = FALSE)


train_data <- read_feather("Shubin/retrain_material/train.feather")
test_data <- read_feather("Shubin/retrain_material/test.feather")
train_data <- train_data[, c(1, 3, 5, 6, 10, 12, 25:30, 138)]
test_data <- test_data[, c(1, 3, 5, 6, 10, 12, 25:30, 138)]

train <- DTcbind(train, train_data, low_mem = TRUE, collect = 1, silent = FALSE)
test <- DTcbind(test, test_data, low_mem = TRUE, collect = 1, silent = FALSE)
