# THIS RUN HAD MANY ISSUES ON FOLD 5!!!!!!!!!!!
...
my_script_is_using <- "E:/Laurae/20161103_lgbm_joost_extreme"
...
ids <- data.table(ID = c(train$Id, test$Id))
...
# Feature Set
source_S30_joost <- fread("Joost/feat_set_source_station_S30.csv")
destination_S30_joost <- fread("Joost/feat_set_destination_station_S30.csv")
source_S32_joost <- fread("Joost/feat_set_source_station_S32.csv")
destination_S32_joost <- fread("Joost/feat_set_destination_station_S32.csv")
source_S34_joost <- fread("Joost/feat_set_source_station_S34.csv")
destination_S34_joost <- fread("Joost/feat_set_destination_station_S34.csv")
source_S35_joost <- fread("Joost/feat_set_source_station_S35.csv")
destination_S35_joost <- fread("Joost/feat_set_destination_station_S35.csv")
source_S36_joost <- fread("Joost/feat_set_source_station_S36.csv")
destination_S36_joost <- fread("Joost/feat_set_destination_station_S36.csv")
source_S30_joost[, ID := NULL]
source_S32_joost[, ID := NULL]
source_S34_joost[, ID := NULL]
source_S35_joost[, ID := NULL]
source_S36_joost[, ID := NULL]
destination_S30_joost[, ID := NULL]
destination_S32_joost[, ID := NULL]
destination_S34_joost[, ID := NULL]
destination_S35_joost[, ID := NULL]
destination_S36_joost[, ID := NULL]
destination_S32_joost[, "S32_dest_-1.0" := NULL]
destination_joost <- cbind(source_S30_joost, source_S32_joost, source_S34_joost, source_S35_joost, source_S36_joost, destination_S30_joost, destination_S32_joost, destination_S34_joost, destination_S35_joost, destination_S36_joost)
rm(source_S30_joost, source_S32_joost, source_S34_joost, source_S35_joost, source_S36_joost, destination_S30_joost, destination_S32_joost, destination_S34_joost, destination_S35_joost, destination_S36_joost)
...
# THE RUN HAD TO BE CANCELLED DURING FOLD 5, SO WE ARE DOING THINGS MANUALLY FROM NOW
...
temp_model_re <- lgbm.train(y_train = NA,
                            x_train = NA,
                            x_test = test,
                            data_has_label = TRUE,
                            NA_value = "nan",
                            lgbm_path = my_lgbm_is_at,
                            workingdir = my_script_is_using,
                            train_name = "lgbm_train_5.csv",
                            val_name = "lgbm_val_5.csv",
                            test_name = "lgbm_test.csv",
                            files_exist = TRUE,
                            predictions = TRUE,
                            log_name = "lgbm_log_5.txt",
                            train_conf = "lgbm_train_5.conf",
                            pred_conf = "lgbm_pred_5.conf",
                            test_conf = "lgbm_test_5.conf",
                            output_preds = "lgbm_predict_5.txt",
                            test_preds = "lgbm_predict_test_5.txt",
                            output_model = "lgbm_model_5.txt",
                            validation = TRUE,
                            verbose = TRUE,
                            num_threads = threads,
                            application = "binary",
...
saveRDS(temp_model_re, file.path(my_script_is_using, "aaa_LightGBM_cv_re.rds"), compress = TRUE)

#temp_model_re <- readRDS(file.path(my_script_is_using, "aaa_LightGBM_cv_re.rds"))

temp_preds_re <- lgbm.predict(model = temp_model_re,
                              data_has_label = TRUE,
                              lgbm_path = my_lgbm_is_at,
                              workingdir = my_script_is_using,
                              input_model = "lgbm_model_5.txt",
                              pred_conf = "lgbm_predict_5.txt",
                              data_name = "lgbm_val_5.csv",
                              files_exist = TRUE,
                              output_preds = "lgbm_predict_5.txt",
                              data.table = TRUE)

saveRDS(temp_preds_re, file.path(my_script_is_using, "aaa_LightGBM_cv_re_pred.rds"), compress = TRUE)

#temp_preds_re <- readRDS(file.path(my_script_is_using, "aaa_LightGBM_cv_re_pred.rds"))

predictedValues <- temp_model$Validation[[1]]
predictedValues[folds[[5]]] <- temp_preds_re

# 635.4 rounds
best_iter <- 0
for (j in 1:5) {
  best_iter <- best_iter + 0.2 * ifelse(j < 5, temp_model$Models[[j]]$Best, 564)
}
...
AnalysisFunc <- function(lgbm, label, folds, validationValues, predictedValuesCV, predictedValues) {
  ...
  if (length(predictedValues) > 1) {
    ...
    best_mcc1_all <- mcc_eval_pred(y_prob = validationValues, y_true = label)
    ...
    best_mcc2_all <- mean(temp_thresh)
    ...
    best_mcc3_all <- (best_mcc1_all + best_mcc2_all) / 2
...
AnalysisFunc(lgbm = NA,
             label = label,
             folds = folds,
             validationValues = predictedValues,
