...
my_script_is_using <- "E:/Laurae/20161028_lgbm_laglead_censored"
...
data1 <- readRDS("Laurae/jayjay_features/Laurae_All_30_60_100_165_censored_mean.rds")
data2 <- readRDS("Laurae/jayjay_features/Laurae_All_30_60_100_165_censored_std.rds")
label <- readRDS("datasets/labels.rds")
data1 <- DTcbind(data1, data2, low_mem = TRUE, collect = 20, silent = FALSE)
train <- DTsubsample(data1, 1:1183747)
test <- DTsubsample(data1, 1183748:2367495)
rm(data1, data2)
gc()
...
AnalysisFunc <- function(lgbm, label, folds, validationValues, predictedValuesCV, predictedValues) {
  ...
  # predictedValues = your prediction values (on test data) on a model trained on all data
  ...
  if (length(lgbm) > 1) {
    ...
    cat("Cross-validated best features list (all used features to copy & paste):\n")
  ...
  cat("Submission with all data by taking the amount of positives of average of overfit+validated threshold positives: ", best_count_extra, ". Threshold=", best_mcc_extra, "\n\n", sep = "")
