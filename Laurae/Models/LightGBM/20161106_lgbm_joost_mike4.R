...
my_script_is_using <- "E:/Laurae/20161106_lgbm_joost_mike4"
...
train <- readRDS("Laurae/20161105_lgbm_joost_extra/train.rds")
test <- readRDS("Laurae/20161105_lgbm_joost_extra/test.rds")
...
ids <- data.table(Id = c(train_mike$Id, test_mike$Id))

# not useful: "sta_31m4" "sta_42m3" "sta_42m4" "sta_46m3" "sta_46m4" "mean_num_value" "max_num_feat" "min_num_value" "min_num_feat"
...
extra_mike <- fread("Mike/0 - Data Files/magic_feature_full_set.csv")
extra_mike <- merge(ids, extra_mike, by = "Id", all.x = TRUE, sort = FALSE)
extra_mike[, Response := NULL]

to_keep <- which(colnames(extra_mike) %in% c("magic3_lead2", 
                                             "magic4_lead2", "magic3_lead3", "magic4_lead3", "magic3_lead4", 
                                             "magic4_lead4", "magic3_lead5", "magic4_lead5", "magic3_lead10", 
                                             "magic4_lead10", "magic3_lead50", "magic4_lead50", "magic3_lead100", 
                                             "magic4_lead100", "magic5_lead2", "magic6_lead2", "magic5_lead3", 
                                             "magic6_lead3", "magic5_lead4", "magic6_lead4", "magic5_lead5", 
                                             "magic6_lead5", "magic5_lead10", "magic6_lead10", "magic5_lead50", 
                                             "magic6_lead50", "magic5_lead100", "magic6_lead100"))
extra_mike <- DTcolsample(extra_mike, to_keep)
gc()

train_mike <- extra_mike[1:1183747, ]
test_mike <- extra_mike[1183748:2367495, ]

train <- DTcbind(train, train_mike, low_mem = TRUE, collect = 1, silent = FALSE)
test <- DTcbind(test, test_mike, low_mem = TRUE, collect = 1, silent = FALSE)
rm(train_mike, test_mike, extra_mike)
gc()
