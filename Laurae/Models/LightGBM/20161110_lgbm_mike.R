...
my_script_is_using <- "E:/Laurae/20161110_lgbm_mike"
...
train <- fread("Mike/0 - Data Files/train_eng.csv")
test <- fread("Mike/0 - Data Files/test_eng.csv")
train[, Id := NULL]
test[, Id := NULL]
train[, Response := NULL]
train <- DTcolsample(train, c(1:5, 14:123), remove = FALSE)
test <- DTcolsample(test, c(1:5, 14:123), remove = FALSE)

for (i in 1:115) {
  train[[i]][train[[i]] == 9999999] <- NA
  test[[i]][test[[i]] == 9999999] <- NA
  gc(verbose = FALSE)
  cat(i, "\n")
}
