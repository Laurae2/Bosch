...
sum_feature <- readRDS("Laurae/NumericFeatures/sum_features.rds")
sum_station_NA <- readRDS("Laurae/NumericFeatures/sum_station_NA.rds")
sum_positive_feature <- readRDS("Laurae/NumericFeatures/sum_positive_features.rds")
positive_sum_station_NA <- readRDS("Laurae/NumericFeatures/positive_sum_station_NA.rds")
sum_negative_feature <- readRDS("Laurae/NumericFeatures/sum_negative_features.rds")
negative_sum_station_NA <- readRDS("Laurae/NumericFeatures/negative_sum_station_NA.rds")
product_feature <- readRDS("Laurae/NumericFeatures/product_features.rds")
product_station_feature <- readRDS("Laurae/NumericFeatures/product_station_features.rds")
sum_sign_feature <- readRDS("Laurae/NumericFeatures/sum_sign_feature.rds")
sum_NA_feature <- readRDS("Laurae/NumericFeatures/sum_na_features.rds")
NA_station_feature <- readRDS("Laurae/NumericFeatures/NA_station_features.rds")


# Clear rare values
temp_NAs <- pbapply(product_station_feature, 2, function(x) {2367495 - sum(x == 0)})
product_station_feature <- product_station_feature[, which(temp_NAs > 48), with = FALSE]
gc()

temp_NAs <- pbapply(negative_sum_station_NA, 2, function(x) {2367495 - sum(x == 0)})
negative_sum_station_NA <- negative_sum_station_NA[, which(temp_NAs > 11), with = FALSE]
gc()


# rescale product_feature because it is too small
temp_NAs <- product_feature == 1
product_feature[!temp_NAs] <- (max(product_feature[!temp_NAs]) - product_feature[!temp_NAs]) / (max(product_feature[!temp_NAs]) - min(product_feature[!temp_NAs]))
product_feature[temp_NAs] <- NA
gc()

# Assign NA to missing values: 15GB peak
sum_feature[sum_feature == 0] <- NA
gc()
sum_station_NA[sum_station_NA == 0] <- NA
gc()
sum_positive_feature[sum_positive_feature == 0] <- NA
gc()
positive_sum_station_NA[positive_sum_station_NA == 0] <- NA
gc()
sum_negative_feature[sum_negative_feature == 0] <- NA
gc()
negative_sum_station_NA[negative_sum_station_NA == 0] <- NA
gc()
product_station_feature[product_station_feature == 0] <- NA
gc()
sum_sign_feature[sum_sign_feature == 0] <- NA
gc()
#sum_NA_feature[sum_NA_feature == 0] <- NA
#NA_station_feature[NA_station_feature == 0] <- NA

gc() #back to 7GB

# 228 variables merge
Numeric_features <- cbind(sum_feature,
                          sum_station_NA,
                          sum_positive_feature,
                          positive_sum_station_NA,
                          sum_negative_feature,
                          negative_sum_station_NA,
                          product_feature,
                          product_station_feature,
                          sum_sign_feature,
                          sum_NA_feature,
                          NA_station_feature)
...
spooky <- numeric(ncol(Numeric_features))
for (i in 1:ncol(Numeric_features)) {
  spooky[i] <- fastdigest(Numeric_features[[i]][1:1183747]) # Compute hash only on training data... obviously
  cat("Iteration ", i, " --- Uniques: ", length(unique(spooky[1:i])), " ---\n", sep = "")
}
Numeric_features <- Numeric_features[, which(!duplicated(spooky)), with = FALSE]
gc() #15GB peak

spooky <- pbapply(Numeric_features, 2, function(x) {r <- ifelse((min(x, na.rm = TRUE) == max(x, na.rm = TRUE)), TRUE, FALSE); gc(verbose = FALSE); r})
gc() #19GB peak
Numeric_features <- Numeric_features[, which(!spooky), with = FALSE]
gc()


# Make feature by train/test: 224 features / 1.2GB
Numeric_features_train <- as.matrix(Numeric_features[1:1183747, ])
gc()
Numeric_features_train <- dropNA(Numeric_features_train)
gc() #24GB peak
Numeric_features_test <- as.matrix(Numeric_features[1183748:2367495, ])
gc()
Numeric_features_test <- dropNA(Numeric_features_test)
gc()
...
Mean_features <- Mean_features[, which(temp_NAs > 35), with = FALSE]
gc()

temp_NAs <- pbapply(Max_features, 2, function(x) {2367495 - sum(is.na(x))})
Max_features <- Max_features[, which(temp_NAs > 35), with = FALSE]
gc()
...
# Transform categoricals appropriately
...
# Proper one-hot encoding using treatment without contrast while handling NAs
...
# Merge temporary features: 270 features
Date_features <- cbind(Date_features, temp_sparse)
rm(temp_sparse)
gc()


# Create date feature: 270 features
...
gc() #37GB peak, back to 17GB



train <- cbind(train, train_mini, Numeric_features_train, Date_features_train) #3.4GB matrix
gc()
...
train <- train[, which(!duplicated(spooky))] # drop to 2.8GB, 533 features
gc()
saveRDS(train, "Laurae/20160930_xgb_superlarge/train_sparse.rds", compress = TRUE)

test <- cbind(test, test_mini, Numeric_features_test, Date_features_test)
gc()
test <- test[, which(!duplicated(spooky))]
saveRDS(test, "Laurae/20160930_xgb_superlarge/test_sparse.rds", compress = TRUE)


# Clean up stuff
rm(Date_features_train, Date_features_test, Numeric_features_train, Numeric_features_test)
gc()

rm(Date_features, Numeric_features)
gc()

rm(train_mini, test_mini, spooky)
gc() #back to 7GB
...
my_learner <- function(depth) {
  ...
  sink(file = "Laurae/20160930_xgb_superlarge/log.txt", append = TRUE, split = FALSE)
  cat("\n\n\nDepth ", depth, "\n\n", sep = "")
  gc()
  set.seed(11111)
