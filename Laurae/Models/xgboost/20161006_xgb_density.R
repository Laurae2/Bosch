...
id1 <- fread("Mike/0 - Data Files/train_man_NFs.csv", header = TRUE, select = 1)
id2 <- fread("Mike/0 - Data Files/test_man_NFs.csv", header = TRUE, select = 1)
id <- rbind(id1, id2)
id$Order <- 1:2367495
rm(id1, id2)
time_data <- fread("Laurae/rodolphe/time_densities.csv", header = TRUE)
time_data <- time_data[, c("V1", "response", "fold_number") := NULL]
time_data <- merge(id, time_data, by = "Id", all.x = TRUE)
time_data <- time_data[order(Order), ]
time_data <- time_data[, c("Id", "Order") := NULL]
gc()
time_data <- as.matrix(time_data)
gc()
time_data <- Matrix(time_data, sparse = TRUE)
gc()
...
gc() #40GB peak
Date_features_train <- cbind(Date_features_train, temp_sparse[1:1183747, ], time_data[1:1183747, ]) #2.6GB, 670 features
gc()
...
gc() #40GB peak
Date_features_test <- cbind(Date_features_test, temp_sparse[1183748:2367495, ], time_data[1183748:2367495, ]) #2.6GB, 670 features
gc()

rm(temp_sparse, time_data)
gc() #19GB used

train <- cbind(train, train_mini, Date_features_train) #3.2GB, 792 features
gc()
...
train <- train[, which(!duplicated(spooky))] #2.9GB, 678 features
gc()
saveRDS(train, "Laurae/20161006_xgb_density/train_sparse.rds", compress = TRUE)

test <- cbind(test, test_mini, Date_features_test) #3.2GB, 789 features
gc()
test <- test[, which(!duplicated(spooky))] #2.9GB, 678 features
saveRDS(test, "Laurae/20161006_xgb_density/test_sparse.rds", compress = TRUE)
...
mcc_eval_nofail_cv <- function(pred, dtrain) {
  ...
  return(list(metric = "mcc", value = ifelse(i < 101, 0.00001*i, round(max(DT[['mcc_v']]), digits = 12)))) #makes xgboost blind for 10 rounds of 5-fold CV
...
mcc_eval_nofail_fold <- function(pred, dtrain) {
  ...
  return(list(metric = "mcc", value = ifelse(i < 251, 0.00001*i, round(max(DT[['mcc_v']]), digits = 12)))) #makes xgboost blind for 125 training rounds
...
my_learner <- function(depth) {
  ...
  sink(file = "Laurae/20161006_xgb_density/log.txt", append = TRUE, split = FALSE)
  cat("\n\n\nDepth ", depth, "\n\n", sep = "")
  gc()
  set.seed(11111)
...
sink(file = "Laurae/20161006_xgb_density/logs_ex.txt", append = TRUE, split = TRUE)
...
for (j in 1:5) {
  ...
  i <- 0
  cat("\n\n\nFold ", j, " on ", format(Sys.time(), "%a %b %d %Y %X"), "\n\n", sep = "")
  sink(file = "Laurae/20161006_xgb_density/logs.txt", append = TRUE, split = FALSE)
  cat("\n\n\nFold ", j, " on ", format(Sys.time(), "%a %b %d %Y %X"), "\n\n", sep = "")
  gc()
  set.seed(11111)
  ...
  write.csv(predictions1, file = "Laurae/20161006_xgb_density/predictions_oof.csv", row.names = FALSE)
  write.csv(predictions3, file = "Laurae/20161006_xgb_density/predictions_test_raw.csv", row.names = FALSE)
  write.csv(features[[j]], file = paste("Laurae/20161006_xgb_density/best_features", j, ".csv", sep = ""), row.names = FALSE)
  
}

predictions2 <- predictions2 / 5
write.csv(predictions2, file = "Laurae/20161006_xgb_density/predictions_test_mean.csv", row.names = FALSE)
...
write.csv(submission0, file = "Laurae/20161006_xgb_density/submission0.csv", row.names = FALSE)
...
write.csv(submission1, file = "Laurae/20161006_xgb_density/submission1.csv", row.names = FALSE)
...
write.csv(submission2, file = "Laurae/20161006_xgb_density/submission2_5.csv", row.names = FALSE)

submission2 <- fread("datasets/sample_submission.csv", header = TRUE, sep = ",", stringsAsFactors = FALSE)
submission2$Response <- as.numeric(best_mcc >= 4)
cat("Submission 2_4 positives:", sum(submission2$Response == 1), "\n\n", sep = "")
write.csv(submission2, file = "Laurae/20161006_xgb_density/submission2_4.csv", row.names = FALSE)

submission2 <- fread("datasets/sample_submission.csv", header = TRUE, sep = ",", stringsAsFactors = FALSE)
submission2$Response <- as.numeric(best_mcc >= 3)
cat("Submission 2_3 positives:", sum(submission2$Response == 1), "\n\n", sep = "")
write.csv(submission2, file = "Laurae/20161006_xgb_density/submission2_3.csv", row.names = FALSE)

submission2 <- fread("datasets/sample_submission.csv", header = TRUE, sep = ",", stringsAsFactors = FALSE)
submission2$Response <- as.numeric(best_mcc >= 2)
cat("Submission 2_2 positives:", sum(submission2$Response == 1), "\n\n", sep = "")
write.csv(submission2, file = "Laurae/20161006_xgb_density/submission2_2.csv", row.names = FALSE)

submission2 <- fread("datasets/sample_submission.csv", header = TRUE, sep = ",", stringsAsFactors = FALSE)
submission2$Response <- as.numeric(best_mcc >= 1)
cat("Submission 2_1 positives:", sum(submission2$Response == 1), "\n\n", sep = "")
write.csv(submission2, file = "Laurae/20161006_xgb_density/submission2_1.csv", row.names = FALSE)
...
write.csv(submission3, file = "Laurae/20161006_xgb_density/submission3.csv", row.names = FALSE)
...
write.csv(submission4, file = "Laurae/20161006_xgb_density/submission4.csv", row.names = FALSE)
