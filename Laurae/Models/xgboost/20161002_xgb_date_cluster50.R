...
gc()
temp_cluster@Dimnames[[2]] <- paste("cluster_n50:", levels(clusters$cluster50), sep = "")
...
train <- cbind(train, train_mini, Date_features_train) #2.3GB matrix of 443 features
gc()
...
train <- train[, which(!duplicated(spooky))] # drop to 2GB, 386 features
gc()
saveRDS(train, "Laurae/20161002_xgb_date_cluster50/train_sparse.rds", compress = TRUE)

test <- cbind(test, test_mini, Date_features_test)
gc()
test <- test[, which(!duplicated(spooky))]
saveRDS(test, "Laurae/20161002_xgb_date_cluster50/test_sparse.rds", compress = TRUE)
...
my_learner <- function(depth) {
  ...
  sink(file = "Laurae/20161002_xgb_date_cluster50/log.txt", append = TRUE, split = FALSE)
  cat("\n\n\nDepth ", depth, "\n\n", sep = "")
  gc()
  set.seed(11111)
