...
Y <- train_numeric_sparse[, train_numeric_sparse@Dim[2]]
train_numeric_sparse <- train_numeric_sparse[, c(1:(train_numeric_sparse@Dim[2] - 1))]
gc()
...
#gc()
test_numeric_sparse <- test_numeric_sparse[, c(1:test_numeric_sparse@Dim[2])]
gc()
...
kept_columns <- c("Id", "L3_S32_F3854:T2", "L3_S32_F3854:T2", "L1_S24_F1695", "L3_S33_F3859", "L3_S32_F3851:T1", "L3_S33_F3865",
...
train <- cbind(train, train_mini, Date_features_train)
gc()
...
train <- train[, which(!duplicated(spooky))]
gc()
saveRDS(train, "Laurae/20160929_xgb_superlarge/train_sparse.rds", compress = TRUE)

test <- cbind(test, test_mini, Date_features_test)
gc()
test <- test[, which(!duplicated(spooky))]
saveRDS(test, "Laurae/20160929_xgb_superlarge/test_sparse.rds", compress = TRUE)
...
my_learner <- function(depth) {
  ...
  sink(file = "Laurae/20160929_xgb_superlarge/log.txt", append = TRUE, split = FALSE)
  cat("\n\n\nDepth ", depth, "\n\n", sep = "")
  gc()
  set.seed(11111)
