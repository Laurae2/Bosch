...
gc()
saveRDS(train, "Laurae/20160928_xgb_superlarge/train_sparse.rds", compress = TRUE)

test <- cbind(test, test_mini, Date_features_test)
gc()
test <- test[, which(!duplicated(spooky))]
saveRDS(test, "Laurae/20160928_xgb_superlarge/test_sparse.rds", compress = TRUE)
...
my_learner <- function(depth) {
  ...
  sink(file = "Laurae/20160928_xgb_superlarge/log.txt", append = TRUE, split = FALSE)
  cat("\n\n\nDepth ", depth, "\n\n", sep = "")
  gc()
  set.seed(11111)
