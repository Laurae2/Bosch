...
dedups <- Matrix(dedups, sparse = TRUE)
gc()

#106 features
train_mini <- cbind(train_mini, dedups[1:1183747, ])
gc()
test_mini <- cbind(test_mini, dedups[1183748:2367495, ])
gc()
...
train <- cbind(train, train_mini, Date_features_train) #2.3GB matrix of 394 features
gc()
...
train <- train[, which(!duplicated(spooky))] # drop to 2GB, 337 features
gc()
saveRDS(train, "Laurae/20161003_xgb_duplicates/train_sparse.rds", compress = TRUE)

test <- cbind(test, test_mini, Date_features_test)
gc()
test <- test[, which(!duplicated(spooky))]
saveRDS(test, "Laurae/20161003_xgb_duplicates/test_sparse.rds", compress = TRUE)
...
my_learner <- function(depth) {
  ...
  sink(file = "Laurae/20161003_xgb_duplicates/log.txt", append = TRUE, split = FALSE)
  cat("\n\n\nDepth ", depth, "\n\n", sep = "")
  gc()
  set.seed(11111)
