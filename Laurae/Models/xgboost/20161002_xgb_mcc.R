...
# Do clusters & Duplicates

clusters <- fread("Laurae/eda_sample_clusters.csv", header = TRUE, colClasses = c("numeric", rep("numeric", 4)), select = 4)[, "cluster_n50", with = FALSE]
clusters$cluster_n50 <- as.factor(clusters$cluster_n50)
colnames(clusters) <- "cluster50"
gc()
temp_cluster <- model.matrix(~ .-1, data = model.frame(clusters))
gc()
dedups <- fread("Laurae/data_dupped4.csv", header = TRUE)
colnames(dedups) <- "dedups"
dedups$dedups <- as.numeric(dedups$dedups)
dedups <- as.matrix(dedups)
temp_cluster <- cbind(temp_cluster, dedups)
temp_cluster <- Matrix(temp_cluster, sparse = TRUE)
gc()
temp_cluster@Dimnames[[2]][1:50] <- paste("cluster_n50:", levels(clusters$cluster50), sep = "")

#157 features
train_mini <- cbind(train_mini, temp_cluster[1:1183747, ])
gc()
test_mini <- cbind(test_mini, temp_cluster[1183748:2367495, ])
gc()

rm(clusters, dedups, temp_cluster)
gc()
...
# Merge temporary features: 320 features
Date_features <- cbind(Date_features, temp_sparse) # peak 14GB
rm(temp_sparse)
gc() # backt to 9GB
...
gc() #35GB peak, back to 13GB



train <- cbind(train, train_mini, Date_features_train) #2.3GB matrix of 444 features
gc()
...
train <- train[, which(!duplicated(spooky))] # drop to 2GB, 387 features
gc()
saveRDS(train, "Laurae/20161002_xgb_mcc/train_sparse.rds", compress = TRUE)

test <- cbind(test, test_mini, Date_features_test)
gc()
test <- test[, which(!duplicated(spooky))]
saveRDS(test, "Laurae/20161002_xgb_mcc/test_sparse.rds", compress = TRUE)
...
my_learner <- function(depth) {
  ...
  sink(file = "Laurae/20161002_xgb_mcc/log.txt", append = TRUE, split = FALSE)
  cat("\n\n\nDepth ", depth, "\n\n", sep = "")
  gc()
  set.seed(11111)
