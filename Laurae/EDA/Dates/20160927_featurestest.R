...
for (i in 1:52) {
  ...
  if (length(listing[[i]]) == 1) {
    feature_Mean[[i]] <- data[, listing[[i]], with = FALSE][[1]]
  } else {
...
for (i in 1:52) {
  ...
  if (length(listing[[i]]) == 1) {
    feature_Min[[i]] <- data[, listing[[i]], with = FALSE][[1]]
  } else {
...
for (i in 1:52) {
  ...
  if (length(listing[[i]]) == 1) {
    feature_Max[[i]] <- data[, listing[[i]], with = FALSE][[1]]
  } else {
...
jpeg(filename = "Laurae/DateFeatures/Extremas_plot.jpg", width = 574, height = 1320, units = "px", pointsize = 6)
tableplot(temporary, select = 1:4, sortCol = 1, nBins = 172, scales = "lin")
dev.off()


# Get first/last station entrance

feature_StationsFirst <- as.numeric(pbapply(feature_ZeroTimeMin, 1, function(x) {r <- which(!is.na(x)); r[1]}))
feature_StationsLast <- as.numeric(pbapply(feature_ZeroTimeMin, 1, function(x) {r <- which(!is.na(x)); r[length(r)]}))
feature_Stations <- data.table(data.frame(FirstStation = feature_StationsFirst, LastStation = feature_StationsLast))
saveRDS(feature_Stations, "Laurae/DateFeatures/StationFirstLast_features.rds")
gc()

temporary <- as.data.frame(cbind(label = Y, feature_Stations[1:1183747, ]))

#tableplot(temporary, select = 1:4, sortCol = 1, nBins = 172, scales = "lin")

jpeg(filename = "Laurae/DateFeatures/FirstLastStations_plot.jpg", width = 574, height = 1320, units = "px", pointsize = 6)
