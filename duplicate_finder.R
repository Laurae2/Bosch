...
write.csv(dupped4, "../input/dupped4.csv", row.names = FALSE)
write.csv(data_dupped4, "../input/data_dupped4.csv", row.names = FALSE)

data_dupped4 = fread( "../input/data_dupped4.csv")
dupped4 = fread("../input/dupped4.csv")
a = df_numeric[data_dupped4$x,]
drop_columns = c()
for(i in colnames(a)){
  if (sum(is.na(a[,i,with = F]))==dim(a)[1]){
    drop_columns = c(drop_columns,i)
  }
}
a = a[,-drop_columns,with = F]
d = a[Response!=-1,]
d[,path:= do.call(paste, c(d[,2:(dim(d)[2]-1),with = F], sep = "")) ]
d[,path:= as.numeric(as.factor(path))]
result = d[,.(count = uniqueN(Response)),by = path]
table(result$count)
